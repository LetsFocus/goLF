...
func InitializeRedis(golf *model.GoLF, prefix string) {
	...
	retryTime, err := time.ParseDuration(golf.Config.Get(prefix + "REDIS_RETRY_DURATION"))
	if err != nil {
	...
	if err != nil && dbStr != "" {
		//golf.Logger.Logger.Errorf("invalid db number: %v", err)
		return
	}
...
func createRedisConnection(config *redisConfig, log slogs.Log) (*redis.Client, error) {
